...
%*************************OFT_ShapiroWilk Methods************************** 
    methods (Access = public)
        
        function pValue = ShapiroWilkForMultipleFreqs(obj, ts, nu)
            %- The m-function, i.e. the function to be minimized in searching for the nFreqsW sinusoids whose removal
            %  most reduces the absolute deviation of the time series.
            [cosPart, sinPart] = EstimateContainedSinusoids(ts, nu);
            plot(ts)
        ...
        %--------------------
        function pValue = ShapiroWilkAlongADirn(obj, x, nuGuessW, dirnW, ts)
           nuTryW = zeros(1,length(nuGuessW));
            for i=1:length(nuGuessW)
                nuTryW(i) = nuGuessW(i) + x * dirnW(i);
            end
            pValue = obj.ShapiroWilkForMultipleFreqs(ts, nuTryW);            
        end
        %--------------------        
        function [ax,bx,cx,foundMaximum,xMax,fxMax] = BracketMaxShapiroWilk(obj, nuGuessW, dirnW, nuMaxCW, ts)
            %- Bracketing for maximization of f(x) = p-value of Shapiro Wilk.
            %  From 10.1 of NumericalRecipes in C.
            %
            % In:
            %    nuGuessW: array of latest frequency index guesses
            %    dirW: frequency index guess direction vector
            %    nuMaxCW:
            %    ts: time series
            % Out:
            %    ax, bx, cx      ax < bx < cx
            %                    f(ax) <= f(bx) >= f(cx) where 
            %                       f(x) =ShapiroWilkAlongADirn(x)
            %       foundMaximum    True iff found maximum instead of brackets for a maximum (due to limits on x)
            %       xMax            Only set if foundMaximum. Value of x for the maximum.
            %       fxMax           Only set if foundMaximum. The maximum value.
            ...
            % this is a simple determination of the tol end threshold.  We
            % could also use the eps(x) fuction to determine the floating
            % point precison
            kScale = 1000;
            kLimitTol = kScale * 0.0005;
            
            xMax = 0;       % xMax, fxMax are only valid if foundMaximim is true
            fxMax = 0;            
            foundMaximum = false;
            ...
            if lo >= hi
                foundMaximum = true;
                xMax = lo;
                fxMax = obj.ShapiroWilkAlongADirn(xMax, nuGuessW, dirnW, ts);
                return
            end
           ...
           scalor = (hi - lo)/kScale;
            ...
            % ensure a -> b is uphill
            fa = obj.ShapiroWilkAlongADirn(ax, nuGuessW, dirnW, ts);
            fb = obj.ShapiroWilkAlongADirn(bx, nuGuessW, dirnW, ts);
            if fb > fa      % a <--> b
                temp = ax;
                ax = bx;
                bx = temp;
            ...
            if cx < lo; cx = lo; end
            if cx > hi; cx = hi; end
            fc = obj.ShapiroWilkAlongADirn(cx, nuGuessW, dirnW, ts);
            
            while true
                               
                if fb - fc > sqrt(eps(fc))  % exit if b->c is downhill
                   return;
                end
               
               % exit if c is up against a limit
               if cx == lo || cx == hi
                   while abs(cx - bx) > kLimitTol * scalor
                       u = 0.5 * (bx + cx);     % mean value
                       fu = obj.ShapiroWilkAlongADirn(u, nuGuessW, dirnW, ts);
                       if fu - fc > sqrt(eps(fc)) % u -> is uphill
                           ax = bx;
                           bx = u;
                           return
                       end
               ...
               if nzDiff < kTiny; nzDiff = kTiny; end  % ensure cannot divide by zero
               if diff < 0; nzDiff = -nzDiff; end
               u = bx - ((bx - cx) * q - (bx - ax) * r) / (2 * nzDiff);
               if u < lo; u = lo; end
               if u > hi; u = hi; end
